...
if(!strcmp($status_grant,"granted"))
{
	if($aflag == 1)
	{
		$result1 = mysql_query("UPDATE op_form SET status = 1 WHERE A_id = '".$staff_id."' AND regno = '".$regno."' AND date = '".$date."' AND  reason = '".$reason."'");
		echo "Request has been granted";
	}
	else 
        { 
                $result2 = mysql_query("UPDATE op_form SET status = 3 WHERE H_id = '".$staff_id."' AND regno = '".$regno."' AND date = '".$date."' AND reason = '".$reason."'");
                echo "Request has been granted";
        }
...
else 
{
        if($aflag == 1)
        { 
                $result3 = mysql_query("UPDATE op_form SET status = 2 WHERE A_id = '".$staff_id."' AND regno = '".$regno."' AND date = '".$date."' AND reason = '".$reason."'");
                echo "Request has been denied";
        }
        else 
        { 
                $result4 = mysql_query("UPDATE op_form SET status = 4 WHERE H_id = '".$staff_id."' AND regno = '".$regno."' AND date = '".$date."' AND reason = '".$reason."'");
                echo "Request has been denied";
        }
